...
def menu_markup():
    ...
    kb.row(types.InlineKeyboardButton("🎮 Open mini app", web_app=types.WebAppInfo(url=f"{config.FRONTEND_URL}/games")))
    kb.row(
        types.InlineKeyboardButton("🧭 My requests", callback_data="menu:status"),
        types.InlineKeyboardButton("linked files", callback_data="menu:files"),
    )
...
def room_markup(game_type: str, game_id: str, player_ids: list[int]):
    ...
    kb.row(types.InlineKeyboardButton("Open game room", web_app=types.WebAppInfo(url=f"{config.FRONTEND_URL}/game/{game_token(game_type, game_id)}")))
    if len(player_ids) >= 2:
        kb.row(
...
def game_invite_markup(game_type: str, game_id: str):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("🎮 Open game room", web_app=types.WebAppInfo(url=f"{config.FRONTEND_URL}/game/{game_token(game_type, game_id)}")))
    return kb
def send_game_invites(game_type: str, game_id: str, players: list[int]):
    url = f"{config.FRONTEND_URL}/game/{game_token(game_type, game_id)}"
...
def cmd_request_status(message):
    ...
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("📊 Open my request status", url=f"{config.FRONTEND_URL}/status/{token}"))
    bot.send_message(message.chat.id, "Your request status:", reply_markup=kb)
@bot.message_handler(commands=["request_dashboard", "dashboard"])
def cmd_request_dashboard(message):
    ...
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("📋 Open admin request dashboard", url=f"{config.FRONTEND_URL}/admin/{token}"))
...
def cmd_profile(message):
    ...
    token = request_status_token(message.from_user.id)
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("📊 My request status", url=f"{config.FRONTEND_URL}/status/{token}"))
    kb.add(types.InlineKeyboardButton("🎮 Open mini app", web_app=types.WebAppInfo(url=f"{config.FRONTEND_URL}/games")))
    text = (
        f"<b>Your profile</b>\n\n"
        f"Name: {escape(fullname(message.from_user))}\n"
        f"Username: {escape('@' + message.from_user.username) if message.from_user.username else 'no username'}\n"
    ...
    try:
        ...
        if data.startswith("menu:"):
            ...
            if page == "back":
                ...
                kb = types.InlineKeyboardMarkup()
                kb.add(types.InlineKeyboardButton("📊 Open my request status", url=f"{config.FRONTEND_URL}/status/{token}"))
                bot.edit_message_text("Your request status:", call.message.chat.id, call.message.message_id, reply_markup=kb)
            return
        ...
        if data.startswith("gs:dash:"):
            ...
            kb = types.InlineKeyboardMarkup()
            kb.add(types.InlineKeyboardButton("📋 Open admin dashboard", url=f"{config.FRONTEND_URL}/admin/{token}"))
...
@app.route("/api/request/dashboard")
...
def run_bot():
    ...
    while True:
        try:
            try:
                me = bot.get_me()
                BOT_USERNAME = me.username or BOT_USERNAME or ""
            except Exception:
                pass
