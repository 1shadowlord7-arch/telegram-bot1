...
def group_settings_markup(chat_id: int):
    ...
    kb.row(
        ...
        types.InlineKeyboardButton("Request settings", callback_data=f"gs:req:{chat_id}"),
    )
    kb.row(
        types.InlineKeyboardButton("Basic word filter", callback_data=f"gs:filter:{chat_id}"),
        types.InlineKeyboardButton("Warning", callback_data=f"gs:warn:{chat_id}"),
    )
    kb.row(
        types.InlineKeyboardButton("Spam detection", callback_data=f"gs:spam:{chat_id}"),
        types.InlineKeyboardButton("my request status", callback_data=f"gs:myreq:{chat_id}"),
    )
    kb.row(types.InlineKeyboardButton("Admin request dashboard", callback_data=f"gs:dash:{chat_id}"))
    kb.row(types.InlineKeyboardButton("Back", callback_data="menu:back"))
    return kb
...
def request_limit_markup(chat_id: int):
    ...
    kb.add(types.InlineKeyboardButton("Back", callback_data=f"gs:open:{chat_id}"))
    return kb
...
def spam_markup(chat_id: int):
    ...
    kb.row(types.InlineKeyboardButton("Back", callback_data=f"gs:open:{chat_id}"))
    return kb
...
def challenge_markup(challenge_id: str):
    ...
    kb.row(types.InlineKeyboardButton("Cancel", callback_data=f"ch:cancel:{challenge_id}"))
    return kb
...
def cmd_warn(message):
    ...
    if not message.reply_to_message:
        bot.reply_to(message, "Reply to a member's message with /warn.")
        return
    ...
    if count >= limit:
        try:
            ...
            pass
        bot.send_message(message.chat.id, f"🚫 {escape(target.first_name)} banned after reaching {count}/{limit} warnings.")
    else:
        bot.send_message(message.chat.id, f"⚠️ {escape(target.first_name)}: {count}/{limit} warnings.")
            ...
            if hit:
                ...
                if count >= warn_limit:
                    try:
                        ...
                        pass
                    bot.send_message(message.chat.id, f"🚫 {message.from_user.first_name} banned after reaching {count}/{warn_limit} warnings.")
                else:
                    bot.send_message(message.chat.id, f"⚠️ {message.from_user.first_name}: {count}/{warn_limit} warnings.")
                return
    ...
    try:
        ...
        if data.startswith("gs:connect:"):
            ...
            if not is_chat_admin(chat_id, call.from_user.id) or not is_bot_admin(chat_id):
                ...
                return
            db.set_state(call.from_user.id, chat_id, "connect_channel", {})
            bot.answer_callback_query(call.id)
            bot.send_message(call.message.chat.id, "Forward a post from the channel you want to connect, or send the channel numeric id.")
            return
        ...
        if data.startswith("gs:filter:"):
            ...
            if not is_chat_admin(chat_id, call.from_user.id) or not is_bot_admin(chat_id):
                bot.answer_callback_query(call.id, "Admin rights required.", show_alert=True)
                return
            db.set_state(call.from_user.id, chat_id, "bad_words", {"words": []})
            bot.answer_callback_query(call.id)
            bot.send_message(call.message.chat.id, "Send one bad word per message, or use underscores like word1_word2_word3. Send /done when finished.")
            return
        ...
        if data.startswith("gs:myreq:"):
            token = request_status_token(call.from_user.id)
            kb = types.InlineKeyboardMarkup()
            kb.add(types.InlineKeyboardButton("Open my request status", web_app=types.WebAppInfo(url=f"{config.BASE_URL}/status/{token}")))
            bot.send_message(call.message.chat.id, "Open your request status here:", reply_markup=kb)
            bot.answer_callback_query(call.id)
            return
        ...
        if data.startswith("gs:dash:"):
            ...
            kb = types.InlineKeyboardMarkup()
            kb.add(types.InlineKeyboardButton("Open admin dashboard", web_app=types.WebAppInfo(url=f"{config.BASE_URL}/admin/{token}")))
        ...
        if data.startswith("ch:"):
            ...
            if game_type in ("ttt", "rps"):
                ...
                game = games.get_game(game_type, game_id)
                bot.edit_message_text(games.result_text(game_type, game_id), call.message.chat.id, call.message.message_id, reply_markup=room_markup(game_type, game_id, game["players"]))
                try:
                    bot.send_message(ch["group_id"], f"{games.game_title(game_type)} room opened.", reply_markup=room_markup(game_type, game_id, game["players"]))
                except Exception:
                    pass
                bot.answer_callback_query(call.id)
                return
